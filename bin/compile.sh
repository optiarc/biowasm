...
do
	...
	cat <<'EOF' >> "${glueCode}.tmp"

// Future direct OPFS hook. Modules that do not implement it explicitly will expose
// a stub so runtimes can detect the capability consistently.
if (typeof Module === "function") {
	const __biowasmModuleFactory = Module;
	Module = function(userModule) {
		userModule = userModule || {};
		return __biowasmModuleFactory(userModule).then(mod => {
			if (!mod.mountOpfs) {
				mod.mountOpfs = async function() {
					throw new Error("This biowasm module does not implement mountOpfs(); rebuild it with direct OPFS support.");
				};
				mod.mountOpfs.__biowasmStub = true;
			}
			mod.biowasmCapabilities = Object.assign({}, mod.biowasmCapabilities, {
				mountOpfs: typeof mod.mountOpfs === "function" && mod.mountOpfs.__biowasmStub !== true,
				mountOpfsSource: mod.mountOpfs && mod.mountOpfs.__biowasmStub === true ? "stub" : "module",
			});
			return mod;
		});
	};
